...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     *
     * The runState provides the main lifecycle control, taking on values:线程池状态
     *
     *   RUNNING:  Accept new tasks and process queued tasks 接受新任务并处理排队任务
     *   SHUTDOWN: Don't accept new tasks, but process queued tasks 不接受新任务，但处理排队任务
     *   STOP:     Don't accept new tasks, don't process queued tasks,不接受新任务，不处理排队任务
     *             and interrupt in-progress tasks 中断正在进行的任务
     *   TIDYING:  All tasks have terminated, workerCount is zero,
     *             the thread transitioning to state TIDYING
     *             will run the terminated() hook method
     *             所有任务都已终止，workerCount为零，线程转换到状态TIDYING将运行terminate（）钩子方法
     *   TERMINATED: terminated() has completed     terminate（）已经完成
     *
     ...
     * (RUNNING or SHUTDOWN) -> STOP  调用shutdown（）时
    ...
    // 再减去1的话，就是 000 11111 11111111 11111111 11111111，前三位代表线程池运行状态runState，
    // 所以这里workerCount的理论最大值就应该是29个1，即536870911
    private static final int CAPACITY   = (1 << COUNT_BITS) - 1;//代表了workerCount的上限

    // runState is stored in the high-order bits 运行状态被储存在高位
    // -1 在Java底层是由32个1表示的，左移29位的话，即111 00000 00000000 00000000 00000000，
    // 也就是低29位全部为0，高3位全部为1的话，表示RUNNING状态，即-536870912
    private static final int RUNNING    = -1 << COUNT_BITS;
    // 0 在Java底层是由32个0表示的，无论左移多少位，还是32个0，即000 00000 00000000 00000000 00000000，
    // 也就是低29位全部为0，高3位全部为0的话，表示SHUTDOWN状态，即0
    private static final int SHUTDOWN   =  0 << COUNT_BITS;
    // 1 在Java底层是由前面的31个0和1个1组成的，左移29位的话，即001 00000 00000000 00000000 00000000，
    // 也就是低29位全部为0，高3位为001的话，表示STOP状态，即536870912
    private static final int STOP       =  1 << COUNT_BITS;
    // 2 在Java底层是由前面的30个0和1个10组成的，左移29位的话，即010 00000 00000000 00000000 00000000，
    // 也就是低29位全部为0，高3位为010的话，表示TIDYING状态，即1073741824
    private static final int TIDYING    =  2 << COUNT_BITS;
    // 2 在Java底层是由前面的30个0和1个11组成的，左移29位的话，即011 00000 00000000 00000000 00000000，
    // 也就是低29位全部为0，高3位为011的话，表示TERMINATED状态，即1610612736；
    private static final int TERMINATED =  3 << COUNT_BITS;

    // Packing and unpacking ctl  封包和拆包 ctl
    private static int runStateOf(int c)     { return c & ~CAPACITY; }
    private static int workerCountOf(int c)  { return c & CAPACITY; }
    //通过“或运算”，将runState和workerCount合并成了一个数字
    ...
    /**
     * 判断是否为RUNNING状态，RUNNING状态为负数，SHUTDOWN为0  RUNNING<SHUTDOWN
     * @param c
     * @return
     */
    ...
    /**
     * 用于保存任务和切换到工作线程的队列。我们不要求workQueue.poll（）返回null就必然意味着workQueue.isEmpty（）
     * 所以完全依靠isEmpty来查看队列是否为空（我们必须这样做，例如在决定是否从SHUTDOWN转换到TIDYING时）
     * 这适用于特殊用途的队列，例如DelayQueues，其中poll（）允许返回null，即使它在延迟时可能稍后返回非null
     *  到期。
    ...
    /**
     * 工厂为新线程。所有线程都是使用此工厂创建的（通过方法addWorker）。
    ...
    /**
     * The default rejected execution handler 默认拒绝执行handler
     */
        ...
        implements Runnable
    {
        ...
        // Lock methods
        //
        // The value 0 represents the unlocked state. 0 未锁
        // The value 1 represents the locked state. 1 锁定
    ...
    /**
     * 如果（SHUTDOWN，池和队列为空）或（STOP和池为空）转换为TERMINATED状态
    ...
    final void tryTerminate() {
        for (;;) {
             ...
             * 1.线程池为运行状态
             * 2.线程池状态大于等于TIDYING
             * 3.线程池状态为SHUTDOWN并且队列不为空
             */
            if (isRunning(c) ||
                runStateAtLeast(c, TIDYING) ||
                (runStateOf(c) == SHUTDOWN && ! workQueue.isEmpty()))
                return;
            if (workerCountOf(c) != 0) { // Eligible to terminate 队列不为空
                interruptIdleWorkers(ONLY_ONE);//只中断一个受阻塞的线程（即空闲线程，正在等待任务）
                return;
            }
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            // 2.如果是SHUTDOWN，而且firstTask不是null，也返回false  shutdown状态：firstTask不为空，不接收新任务
            // 3.如果是SHUTDOWN，firstTask是null，而且任务队列是空，返回false shutdown状态：firstTask为空，不接收新任务
            // 简单分析下：
            ...
            //当线程池状态为RUNNING时
            for (;;) {
                //获取workQueue队列中worker的个数
                int wc = workerCountOf(c);
                //当wc大于默认workerCount的上限或者大于制定corePoolSize大小，返回false
                if (wc >= CAPACITY ||
                    wc >= (core ? corePoolSize : maximumPoolSize))
                    return false;
                //增加WorkerCount的个数，成功后跳出循环，创建线程
                if (compareAndIncrementWorkerCount(c))
                    break retry;
                //由于并发原因，再次获取ctl的值
                c = ctl.get();  // Re-read ctl
                //正常如果CAS失败，只要在当前内部for循环重试即可，但是为了防止其他线程
                //修改线程池状态，需要判断当前线程池是否还是和上次获取的线程池状态相同，
                //如果不同，回到retry:标记的最外层for循环重新判断
                if (runStateOf(c) != rs)
                    continue retry;
        ...
        Worker w = null;
        try {
            //初始化Worker实例，将firstTask交给Worker中的线程thread
            ...
            if (t != null) {
                ...
                try {
                    ...
                    //持有锁时重新查state,退出ThreadFactory失败或在锁定获取之前关闭。
                    int rs = runStateOf(ctl.get());
     ...
     *   添加工作线程失败，如果失败意味着已经饱和或者被 shutdown 了
     *
     */
    private void addWorkerFailed(Worker w) {
        ...
        try {
            ...
            decrementWorkerCount();//减少workcount
            tryTerminate();
        } finally {
            mainLock.unlock();
        }
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        //1.如果是false，说明是非核心线程没有接受到任务而退出，在getTask中已经对
        //workcount进行了调整，所以不用-1
        //2.如果是true,说明是task.run（）发生异常，突然退出，需要调整workcount的值
     ...
     * 获取任务方法的几种可能性：
     * 1.可能返回null:1-> 线程池状态为shutdown并且任务队列为空，这种不再接受新的任务
     *                2-> 线程池状态为stop状态以上
     *                3-> 线程池中有大于 maximumPoolSize 个 workers 存在
     *                    (通过调用 setMaximumPoolSize 进行设置,此worker中断重试)
     *                4-> 允许核心线程数内的线程回收，或当前线程数超过了核心线程数，当获取任务超时的时候
     *                    任务队列为空或者线程至少有一个在运行的时候返回null
     */
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            // Check if queue empty only if necessary.
            // 判断线程池状态：
            // 1、线程池状态为shutdown并且任务队列为空,如果有任务,线程池还可以继续处理
            // 2、线程池状态为stop状态以上
            ...
            /**
             * 1、线程数大于maximumPoolSize(什么时候会出现这种情况？ 当其他线程通过set方法对其进行修改)
            ...
            if ((wc > maximumPoolSize || (timed && timedOut))
                && (wc > 1 || workQueue.isEmpty())) { //在阻塞队列不为空时，需要保证至少有一个worker
            ...
            try {
                ...
                //通过调用 setMaximumPoolSize 进行设置,此worker会发生中断重试),为什么会中断具体看setMaximumPoolSize
                timedOut = false;
            }
        }
    }
    ...
    final void runWorker(Worker w) {
         ...
         */
        w.unlock(); // allow interrupts // 修改state为0，将占用锁的线程设为null（第一次执行之前没有线程占用）
        ...
        try {
            ...
            //如果进行到这里，说明非核心线程在KeepAlive时间内没有获取到任务，此worker退出线程池
    ...
    /**
     * 根据给定的初始化参数创建一个ThreadPoolExecutor,具备默认线程工厂
     * 和拒绝handler
    ...
    public void execute(Runnable command) {
        ...
        //获取ctl值
        int c = ctl.get();
        //比较workercount值和corePoolSize大小 即线程数和基本线程数
        ...
        // 如线程数大于等于基本线程数或线程创建失败
        //根据c判断当前线程池的状态是否为RUNNING状态，即既可以接受新任务，又会处理队列任务的状态
        //并且通过offer()方法，尝试将commond添加到队列workQueue中,即如果BlockingQueue可以容纳，则返回true，否则返回false
        ...
        //如果尝试添加新的worker线程处理任务command失败，调用reject()方法拒绝任务command，
        // 这时线程数的判断利用maximumPoolSize作为边界约束条件
